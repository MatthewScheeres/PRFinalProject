...
from tensorflow.keras.layers import Conv2D, LeakyReLU, Activation, Concatenate, Conv2DTranspose
...
def generate_final_images(g_model, trainX, name, n_samples=10):
	x = 0
	y = int(trainX.shape[0]/2)
	while x < (y):
		upper = min(x+n_samples, y)
		ix = np.arange(x, upper, 1)
		X_in = trainX[ix]
		# generate translated images
		X_out, _ = generate_fake_samples(g_model, X_in, 0)
		X_in = (X_in + 1) / 2.0
		...
		for i in range(X_in.shape[0]):
			pyplot.subplot(4, plotCols, plotCols*(i//plotCols) + i%plotCols + 1)
			pyplot.axis('off')
			pyplot.imshow(X_in[i])
		# plot translated image
		for i in range(X_in.shape[0]):
			pyplot.subplot(4, plotCols, plotCols*(i//plotCols) + i%plotCols + 1 + n_samples)
			pyplot.axis('off')
			pyplot.imshow(X_out[i])
			# save plot to file
		filename1 = 'FinalImages/%s_final_plot_%i-%i.png' % (name, x, upper)
		pyplot.savefig(filename1)
		pyplot.close()
		x = x+n_samples
	...
	with open("log.txt", "w") as logFile:
		with open("log.csv", "w") as  csvFile:
			...
			for i in range(n_steps):
				...
				# print('>%d, dA[%.3f,%.3f] dB[%.3f,%.3f] g[%.3f,%.3f]' % (i+1, dA_loss1[0],dA_loss2[0], dB_loss1[0],dB_loss2[0], g_loss1,g_loss2))
				# logFile.write('>%d, dA[%.3f,%.3f] dB[%.3f,%.3f] g[%.3f,%.3f]\n' % (i+1, dA_loss1[0],dA_loss2[0], dB_loss1[0],dB_loss2[0], g_loss1,g_loss2))
			...
			# generate a batch of the training set at the end of training
			#generate_final_images(g_model_AtoB, trainA, 'AtoB')
			#generate_final_images(g_model_BtoA, trainB, 'BtoA')
